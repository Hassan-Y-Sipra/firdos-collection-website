...
const Chakeout = () => {
  ...
  console.log("Product being sent for payment:", product);
  ...
  //     const response = await fetch("https://firdos-collection-website.onrender.com/create-order", {
  //       method: "POST",
  //       headers: {
  ...
  const handlePayment = async (product) => {
    ...
    console.log("Fetched user_id:", localStorage.getItem("user_id"));


    if (!product || !product.id || !product.price || !userId) {
      alert("Invalid product details!");
      return;
    }
